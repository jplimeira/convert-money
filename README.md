# 💱 Convert Money | Conversor de Moedas

Aplicação web para conversão de moedas, permitindo converter valores em Real (BRL) para diferentes moedas internacionais.

<img width="1084" height="904" alt="image" src="https://github.com/user-attachments/assets/64f4ebb4-8603-417b-a3a0-cd1cd458d904" />

---

## 🚀 Funcionalidades

- Conversão de **Real (BRL)** para:
  - 💵 Dólar (USD)
  - 💶 Euro (EUR)
  - 💷 Libra (GBP)
  - ₿ Bitcoin (BTC)
- Atualização dinâmica da moeda selecionada
- Exibição formatada dos valores convertidos
- Interface intuitiva e de fácil utilização
- Favicon personalizado

---

## 🖥️ Tecnologias utilizadas

- HTML5 (estrutura)
- CSS3 (estilização e layout)
- JavaScript (lógica e interatividade)

---

## 🎨 Layout

Design moderno com foco em:
- Simplicidade
- Usabilidade
- Clareza na conversão de valores

---

## 📂 Estrutura do projeto

```
📁 convert-money
 ┣ 📁 assets
 ┣ 📄 index.html
 ┣ 📄 style.css
 ┣ 📄 scripts.js
```

---

## ⚙️ Como executar o projeto

1. Clone o repositório:
```
git clone https://github.com/jplimeira/convert-money.git
```

2. Acesse a pasta:
```
cd convert-money
```

3. Abra o arquivo:
```
index.html
```

---

## ⚠️ Observações

- As cotações utilizadas são valores fixos e servem apenas para fins de demonstração.
- O projeto pode ser evoluído com integração a APIs de cotação em tempo real.

---

## 🔮 Melhorias futuras

- Integração com API de cotação em tempo real
- Responsividade para dispositivos móveis
- Melhorias de UX/UI
- Suporte à conversão entre diferentes moedas internacionais (ex: USD → EUR, EUR → GBP, etc.)

---

## 👨‍💻 Autor

Desenvolvido por Jayro Pereira 🚀

---

## 📌 Status do projeto

✅ Concluído (versão inicial)  
🚧 Em desenvolvimento
